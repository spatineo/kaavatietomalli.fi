import { Translations } from './types';

export const fi: Translations = {
  common: {
    backToHome: 'Etusivulle',
    loadMore: 'Lataa lisää',
    loading: 'Ladataan...',
    loadingChart: 'Ladataan kaaviota...',
    all: 'Kaikki',
    themes: 'Teemat',
    author: 'Päätoimittaja',
    rights: 'Oikeudet',
    contact: 'Yhteys',
    skipToContent: 'Hyppää sisältöön',
    footerTitle: 'Kaavatietomalli.fi',
    footerText: 'Tietomallimuotoisen alueidenkäytön suunnittelun one-stop shop.',
  },
  navigation: {
    blog: 'Blogi',
    home: 'Etusivu',
    newest: 'Uusimmat',
    closeMenu: 'Sulje valikko',
    openMenu: 'Avaa valikko',
    mainNav: 'Päänavigaatio',
    githubRepo: 'GitHub-repositorio',
  },
  hero: {
    subtitle: 'Tietomallimuotoinen kaavoitus meillä ja muualla',
    titleMain: 'Kaava',
    titleAccent: 'tieto',
    titleMalli: 'malli',
    description: 'Digitalisoituvan alueidenkäytön suunnittelun tarina: historia, uusi ajattelumalli, yhteentoimivuus, tekniset ratkaisut ja kehitysideat',
    editorChief: 'Päätoimittaja',
  },
  blog: {
    sectionSubtitle: 'Rakentavasti rakenteistamisesta',
    titleMain: 'Kaava',
    titleAccent: 'tieto',
    titleBlogi: 'blogi',
    description: 'Merkintöjä digitalisoituvan rakennetun ympäristön mahdollistajilta. Tekstit edustavat kirjoittajien henkilökohtaisia mielipiteitä.',
    relatedArticles: 'Liittyvät artikkelit',
    topic: 'Aihepiiri',
    timelineAria: 'Artikkelien aikajana',
  },
  history: {
    sectionSubtitle: 'Kaavatietomallin historia',
    titleMain: 'Missä sitä',
    titleAccent: 'ollaan',
    titleOlta: 'oltu?',
    description: 'Poimintoja tietomallimuotoisen kaavoituksen kehittämisen elinkaarelta.',
    exploreData: 'Tutustu',
  },
  search: {
    title: 'Haku',
    placeholder: 'Hae artikkeleita, sivuja...',
    noResults: 'Ei tuloksia haulle',
    close: 'Sulje',
    types: {
      post: 'Artikkeli',
      page: 'Sivu',
      author: 'Kirjoittaja',
      class: 'Luokka',
      codelist: 'Koodisto',
    },
  },
  interactiveImage: {
    expand: 'Laajenna kuvaaja',
    zoomIn: 'Lähennä',
    zoomOut: 'Loitonna',
    zoomToFit: 'Mahduta',
    close: 'Sulje',
  },
  mermaid: {
    renderError: 'Mermaid-kaavion piirto epäonnistui',
  },
  post: {
    returnToList: 'Palaa artikkelilistaukseen',
    previous: 'Edellinen',
    next: 'Seuraava',
    noPrevious: 'Ei edellistä',
    noNext: 'Ei seuraavaa',
    author: 'Kirjoittaja',
    ariaLabel: 'Lue artikkeli',
    readMore: 'Lue lisää',
    illustrationAlt: 'Kuvituskuva',
    comments: 'Keskustelu',
    showComments: 'Näytä keskustelu',
    hideComments: 'Piilota keskustelu',
    commentCount: '{{count}} kommenttia',
    latestComment: 'Viimeisin: {{date}}',
    relatedPostsSection: 'Lisää aihepiiristä',
    commercialCooperation: 'Kaupallinen yhteistyö',
    commercialCooperationLabel: 'Kaupallinen yhteistyö {{partner}}',
    commercialCooperationIntro: 'Tämä artikkeli on tuotettu yhteistyössä kumppanimme kanssa.',
    aboutCommercialCooperation: 'Tietoa kaupallisesta yhteistyöstä',
    aboutCommercialCooperationText: 'Tämä kirjoitus on osa kaupallista yhteistyötä Kaavatietomalli.fi-sivuston ja {{partner}} välillä. Kaikki yhteistyösisällöt valitaan huolella ja ne palvelevat lukijoitamme syventämällä ymmärrystä digitalisoituvasta maankäytön suunnittelusta. Haluatko yrityksesi asiantuntijuuden esille sivustollemme? Ota yhteyttä!',
    readMoreCooperation: 'Lue lisää yhteistyöstä',
    callToActionLabel: 'Tutustu tarkemmin',
  },
  page: {
    tableOfContents: 'Sisällysluettelo',
    close: 'Sulje',
    linkToSection: 'Linkki tähän osioon: {{text}}',
  },
  author: {
    contactInfo: 'Yhteystiedot',
    specialties: 'Erikoisalat',
    cooperationTitle: 'Haluatko keskustella yhteistyöstä?',
    cooperationText: 'Ilkka ja Spatineon asiantuntijat auttavat organisaatiotanne hyödyntämään rakennetun ympäristön tietoa tehokkaammin.',
    contactUs: 'Ota yhteyttä',
    sendEmail: 'Lähetä sähköpostia',
  },
  notFound: {
    title: 'Sitäpä ei nyt löytynyt',
    message: 'Hae sivuston muusta sisällöstä',
    searchPrompt: 'Hae artikkeleita, sivuja...',
    backToHome: 'Palaa etusivulle',
    redirecting: 'Ohjataan uudelleen...',
    redirectingDescription: 'Etsimääsi sivua ei löytynyt suoralla osoitteella. Ohjaamme sinut sivuston hakuun ja sisällönluetteloon.',
    redirectingClickHere: 'Jos ohjaus ei tapahdu automaattisesti, klikkaa tästä.',
  },
  consent: {
    title: 'Evästeasetukset',
    description: 'Käytämme evästeitä parantaaksemme käyttökokemustasi ja analysoidaksemme sivuston käyttöä. Voit muokata asetuksiasi alta.',
    acceptAll: 'Hyväksy kaikki',
    rejectAll: 'Vain välttämättömät',
    customize: 'Muokkaa asetuksia',
    save: 'Tallenna valinnat',
    essentials: {
      title: 'Välttämättömät evästeet',
      description: 'Nämä evästeet ovat välttämättömiä sivuston toiminnalle, eikä niitä voi poistaa käytöstä.',
    },
    analytics: {
      title: 'Analytiikka-evästeet',
      description: 'Näiden evästeiden avulla seuraamme sivuston kävijämääriä ja käyttöä parantaaksemme palvelua.',
    },
  },
  llms: {
    note: 'Note: The content of this website is primarily written in Finnish.',
    links: 'Links',
    articlesFeatured: 'Articles (Featured)',
    articles: 'Articles',
    pages: 'Pages',
    optional: 'Optional',
    fullIndexTitle: 'Full Content Index',
    fullIndexDescription: 'Comprehensive index of all articles and pages for LLMs.',
    fullIndexNote: 'Full index of all articles and pages is available at [llms-full.txt](${baseUrl}/llms-full.txt).',
  },
  versionMismatch: {
    title: 'Päivitys saatavilla',
    description: 'Sivustoa on muokattu tai sinne on julkaistu uutta sisältöä.',
    button: 'Lataa sivusto uudelleen',
    buttonLater: 'Päivitä myöhemmin',
  },
  geojson: {
    interactiveMapBlock: 'Karttanäkymä',
    darkTheme: 'Tumma',
    lightTheme: 'Vaalea',
    darkThemeTooltip: 'Tumma karttatyyli',
    lightThemeTooltip: 'Vaalea karttatyyli',
    mapTab: 'Kartta',
    codeTab: 'Koodi',
    exitFullscreen: 'Lopeta kokoruutu',
    enterFullscreen: 'Kokoruutu',
    jsonParseError: 'Virheellinen JSON-rakenne',
    clickShapesForPopups: 'Klikkaa karttakohteita nähdäksesi ominaisuustiedot',
    fullscreenStatus: 'KOKORUUTU',
    dynamicStatus: 'UPOTETTU',
    propertiesTitle: 'Kohteen tiedot',
    reprojectedSuffix: 'Reprojisoitu',
    ogcStandardLabel: 'OGC-standardi',
    formatLabel: 'Formaatti',
    crsLabel: 'Koordinaatisto',
  },
  video: {
    consentRequired: 'Videon toistaminen edellyttää seurantaevästeiden hyväksymistä, koska ulkopuoliset palvelut (YouTube ja Vimeo) saattavat asettaa evästeitä tai muulla tavoin seurata käyttöä.',
    enableAnalytics: 'Hyväksy seuranta ja näytä video',
  },
  errorBoundary: {
    title: 'Pahoittelut, tapahtui virhe',
    description: 'Sisällön esittämisessä tapahtui odottamaton virhe.',
    button: 'Palaa turvallisesti etusivulle',
    blockError: 'Virhe elementin esittämisessä',
  },
  dataModel: {
    dataModelBrowser: 'Tietomalliselain',
    description: 'Kuvaus',
    noDescription: 'Ei kuvausta saatavilla valitulla kielellä.',
    documentationAndHistory: 'Dokumentaatio',
    modelDetails: 'Mallin tiedot',
    version: 'Versio',
    status: 'Tila',
    modified: 'Muokattu',
    browseClassesAndCodelists: 'Tutki tietomallin luokkia ja koodistoja',
    selectClassOrCodelistHelp: 'Valitse luokka tai käytössä oleva koodisto nähdäksesi sen tarkemmat tiedot.',
    classesOptGroup: 'Sisältyvät luokat',
    codelistsOptGroup: 'Käytetyt koodistot',
    classLabel: 'Luokka',
    codelistLabel: 'Koodisto',
    technicalName: 'Tekninen nimi:',
    uri: 'URI:',
    classDiagram: 'Luokkakaavio',
    attributes: 'Attribuutit',
    name: 'Nimi',
    technicalNameOrId: 'Tekninen nimi / ID',
    dataType: 'Tietotyyppi',
    cardinality: 'Monikertaisuus / pakollisuus',
    associations: 'Assosiaatiot',
    roleOrAssociationName: 'Rooli / Assosiaation nimi',
    targetClass: 'Kohdeluokka',
    definition: 'Määritelmä',
    codelistStatus: 'Koodiston tila:',
    updated: 'Päivitetty:',
    openCodelistService: 'Koodistot-palvelu',
    codesTitle: 'Koodit',
    codesCountSuffix: 'kpl',
    codeValue: 'Koodiarvo',
    copyUri: 'Kopioi URI',
    copied: 'Kopioitu',
    noCodesAvailable: 'Koodistolla ei ole koodeja tai ne eivät ole saatavilla.',
    loadingCodelistInfo: 'Ladataan koodiston tietoja...',
    errorLoadingCodelist: 'Virhe haettaessa koodiston tietoja tai koodistoa ei löydy.',
    versionOptionLabel: 'Versio',
    originSyncTimeLabel: 'Noudettu Yhteentoimivuusalustalta:',
    showDocumentation: 'Näytä dokumentaatio',
    hideDocumentation: 'Piilota dokumentaatio',
    filterCodesPlaceholder: 'Suodata koodeja',
    statusValid: 'Voimassa',
    statusDraft: 'Luonnos',
    statusSuperceded: 'Korvattu',
    statusRetired: 'Poistettu käytöstä',
    statusInvalid: 'Virheellinen',
    statusSuggested: 'Ehdotus',
    statusIncomplete: 'Keskeneräinen',
    statusHidden: 'Piilotettu'
  },
  callouts: {
    note: 'Huomautus',
    warning: 'Varoitus',
    tip: 'Tärkeää',
    info: 'Huomaa',
  },
  validation: {
    subtitle: 'Ryhti-validointi: Kaavasuunnitelma',
    title: 'Kaavasuunnitelman validointityökalu',
    apiInfoDesc: 'Tämä työkalu tarkistaa JSON-muotoisen kaavasuunnitelman Ryhti-järjestelmän avoimen validointirajapinnan avulla. Sovellus ei talleta lähetettyjä tietoja pysyvästi. Älä kuitenkaan koskaan lähetä mitään henkilötietosisältöä validoitavaksi. Validaattorin käyttämiseen tarvitset ilmaisen Syken kehittäjätilin ja henkilökohtaisen API-avaimen.',
    environment: 'Ryhti-ympäristö',
    apiKey: 'API-avain',
    apiKeyPlaceholder: 'Kopioi API-avaimesi tähän',
    planType: 'Kaavalaji',
    areaIdentifier: 'Kunta- tai maakuntakoodi',
    areaPlaceholder: 'Esim. 049 tai 091',
    editorLabel: 'Validoitava Kaava-JSON-dokumentti',
    buttonLoadFile: 'Lataa tiedosto',
    buttonFormat: 'Muotoile JSON',
    buttonExample: 'Palauta esimerkki',
    buttonValidate: 'Suorita validointi',
    loading: 'Validoidaan tietosisältöä...',
    noResult: 'Odottaa validointia',
    noResultDesc: 'Aseta tarvittavat parametrit ja käynnistä validointi',
    successTitle: 'Ei virheitä',
    successDesc: 'Tietorakenne ja sisältö vastaavat Ryhti-kaavasuunnitelman vaatimuksia. Ei virheitä tai varoituksia.',
    errorTitle: 'Validointi löysi huomautettavaa',
    errorDesc: 'Tietosisällöstä tai rakenteesta löytyi virheitä tai varoituksia.',
    rawResponse: 'Raaka API-vastaus',
    copyBtn: 'Kopioi vastaus',
    copiedBtn: 'Kopioitu!',
    invalidJson: 'Virheellinen JSON-muoto',
    missingParams: 'Täytä API-avain, hallintoalueen koodi ja varmista, että JSON-dokumentti on oikein muotoiltu.',
    analyzingResponse: 'Analysoidaan...',
    connectionOrServerError: 'Yhteys- tai palvelinvirhe',
    auditSummary: 'Validoinnin yhteenveto',
    status: 'Tila',
    timestamp: 'Aikaleima',
    endpointAndInterface: 'Yhteyspiste & Rajapinta',
    ryhtiTest: 'Ryhti testi (api-test.ymparisto.fi)',
    ryhtiProd: 'Ryhti tuotanto (api.ymparisto.fi)',
    rulesPassed: 'Säännönmukaisuustarkastus läpäisty:',
    schemaValid: 'Skeeman rakenne ja muotoilu ovat täysin valideja.',
    observedPrefix: 'Havaittu ',
    observedObjects: ' kaavakohdetta (planObjects) ja ',
    observedGroups: ' määräysryhmää (planRegulationGroups).',
    requirementsMet: 'Pakolliset attribuutit, koodistoviittaukset ja relaatiot vastaavat Ryhti-vaatimuksia.',
    selectedEditorPoint: 'Valittu kohta editorissa',
    line: 'Rivi',
    issue: 'huomio',
    issues: 'huomiota',
    warning: 'Varoitus',
    error: 'Virhe',
    noIssuesOnLine: 'Rivillä {{line}} ei ole havaittuja sääntörikkeitä.',
    clickLineToSeeDetails: 'Klikkaa korostettua riviä nähdäksesi tiedot.',
    selectAnotherLineDesc: 'Valitse jokin toinen korostetuista (punainen) riveistä nähdäksesi siihen kohdistuvat virheet ja varoitukset.',
    clickHighlightedLineDesc: 'Klikkaa mitä tahansa korostettua koodiriviä tai alla olevia pikavalintoja avataksesi kyseisen kohdan sääntörikkeet tähän raporttipaneeliin.',
    linesWithIssues: 'Rivit, joissa huomioita:',
    tryOwnKeys: 'Haluatko kokeilla omilla avaimilla?',
    tryOwnKeysDesc: 'Voit luoda maksuttoman kehittäjätilin ja tilata omat koetunnuksesi Suomen ympäristökeskuksen rajapintaportaalista osoitteesta ',
    showRawResponseJson: 'Näytä raaka API-vastaus (JSON)',
    httpStatus: 'Vastauksen tilakoodi:',
    hide: 'Piilota ▲',
    show: 'Näytä ▼',
    generalFallback: 'Yleinen (General)',
    unknownError: 'Tuntematon virhe',
    unknownWarning: 'Tuntematon varoitus',
    networkError: 'Verkkoyhteysvirhe: Ryhti-rajapintaan ei saatu yhteyttä. Varmista, että internet-yhteytesi toimii.',
    valid: 'Validi',
    targetProperty: 'Raportoitu kenttä',
    deserializationFailure: 'Palvelu ei voinut muodostaa tietorakennetta ({{dto}}). Tämä tarkoittaa yleensä, että kohteesta puuttuu pakollisia tietoja, siinä on tuntemattomia kenttiä tai sisällön muoto on väärä.',
    rootDeserializationFailure: 'Koko kaavadokumentin (plan / planDto) muodostaminen epäonnistui. Varmista, että juuritason pakolliset kentät ovat oikein ja ylimääräisiä kenttiä ei ole.',
    showAllIssues: 'Näytä kaikki huomiot',
    savedToBrowserMemory: 'pysyy selaimen paikallisessa muistissa',
    environmentProduction: 'Tuotanto',
    environmentTest: 'Testi'
  },
  planBrowser: {
    title: 'Kaavaselain',
    subtitle: 'Selaa Ryhti-järjestelmään vietyjä voimassa olevien kaavojen hakemistoja. Tiedoissa saattaa olla virheitä ja puutteita, ne haetaan sellaisenaan Ryhti-palvelun avoimesta paikkatietorajapinnasta. Huomaa, että kaavan tiedot sisältävät tässä vaiheessa vain ulkorajan, ei kaavakohteita tai -määräyksiä. Kaavatiedot / Lähde: Syke. Kuntarajat / Lähde: Maanmittauslaitoksen Maastotietokanta.',
    searchPlanPlaceholder: 'Hae kaavan nimellä...',
    searchPlanPlaceholderLabel: 'Hae kaavan nimellä',
    filterMunicipalityPlaceholder: 'Suodata kunnan mukaan...',
    allMunicipalities: 'Kaikki kunnat ja kaupungit',
    allPlanTypes: 'Kaikki kaavalajit',
    filterByMapBounds: 'Hae vain kartan alueelta',
    showingPlansCount: 'Haettu {loaded}/{total} kaavaa',
    noPlansFound: 'Kaavoja ei löytynyt',
    noPlansFoundDesc: 'Kokeile muuttaa hakuehtoja tai tyhjennä suodattimet.',
    clearFilters: 'Tyhjennä suodattimet',
    planDetails: 'Kaavan tiedot',
    selectPlanPrompt: 'Valitse kaava listalta',
    selectPlanPromptDesc: 'Napsauta mitä tahansa kaavaa nähdäksesi sen rajauksen kartalla ja tarkemmat ominaisuustiedot.',
    basicInfo: 'Perustiedot',
    timeline: 'Voimassaolo ja aikataulu',
    description: 'Kuvaus',
    documents: 'Asiakirjat ja liitteet',
    rawGeoJson: 'GeoJSON-raakadata',
    municipality: 'Kunta tai kaupunki',
    planType: 'Kaavalaji',
    permanentId: 'Pysyvä kaavatunnus',
    producerId: 'Kuntakohtainen tunnus',
    approvalDate: 'Hyväksyminen',
    validityDate: 'Voimassaolo',
    initiationDate: 'Vireilletulo',
    lifeCycleStatus: 'Elinkaaren tila',
    viewDocument: 'Avaa asiakirja',
    loadingPlans: 'Ladataan kaavatietoja...',
    loadingMunicipalities: 'Ladataan kuntarajoja...',
    zoomToPlan: 'Tarkenna kaavaan kartalla',
    selectMunicipalityPrompt: 'Valitse kunta...',
    selectMunicipalityFirst: 'Käytä tekstihakua tai valitse kunta kaavojen hakemiseen.',
    loadMorePlans: 'Hae lisää',
    loadingMorePlans: 'Ladataan...',
    allPlansLoaded: 'Kaikki kaavat ladattu',
    back: 'Takaisin',
    exitFullscreen: 'Poistu koko ruudun tilasta',
    enterFullscreen: 'Koko ruutu',
    fetchError: 'Virhe ladattaessa kaavatietoja WFS-rajapinnasta',
    loadMoreError: 'Virhe ladattaessa seuraavaa erää',
    darkMap: 'Tumma',
    lightMap: 'Vaalea',
    municipalityBoundary: 'Kuntaraja',
    zoomToMunicipality: 'Tarkenna valittuun kuntaan',
    defaultPlanName: 'Asemakaava',
    defaultPlanType: 'Kaava',
    noDocuments: 'Ei erillisiä asiakirjaliitteitä',
    noDocumentsDesc: 'Tälle kaavalla ei ole linkitetty erillisiä PDF-asiakirjoja.',
    downloadGeoJson: 'Lataa GeoJSON',
    countUnit: 'kpl',
    detailedPlanLayer: 'Asemakaavat',
    masterPlanLayer: 'Yleiskaavat'
  },
};
